...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "sqlite-full-cycle-batch-1",
                "trace_id": "sqlite-full-cycle-batch-trace-1",
            ...
            response = {
                "request_id": "sqlite-full-cycle-1",
                "trace_id": "sqlite-full-cycle-trace-1",
...
class SqliteFullCycleTest(unittest.TestCase):
    def setUp(self) -> None:
        MockState.requests = {}
        ...
        config = {
            "agent_name": "sqlite-full-cycle-test",
            "log_level": "INFO",
            "denotary": {
            ...
            "sources": [
                {
                    ...
                    "backfill_mode": "full",
                    "connection": {"path": str(self.database_path)},
                    "options": {
                        ...
                        "dry_run_events": [
                            {
                                ...
                                "operation": "snapshot",
                                "primary_key": {"id": 101},
                                "change_version": "main.invoices:2026-04-20T09:00:00Z:101",
                                "checkpoint_token": '{"main.invoices":{"watermark":"2026-04-20T09:00:00Z","pk":[101]}}',
    ...
    def test_run_once_executes_sqlite_full_cycle_and_exports_proof_bundle(self) -> None:
        ...
        deliveries = engine.store.list_deliveries("sqlite-edge-core")
        self.assertEqual(len(deliveries), 1)
        self.assertEqual(deliveries[0]["status"], "finalized_exported")
        self.assertEqual(deliveries[0]["tx_id"], "c" * 64)

        proofs = engine.store.list_proofs("sqlite-edge-core")
        ...
        self.assertEqual(proof_payload["request_id"], "sqlite-full-cycle-1")
        self.assertEqual(proof_payload["receipt"]["tx_id"], "c" * 64)
        self.assertEqual(proof_payload["audit_chain"]["record"]["status"], "finalized")

    def test_run_once_executes_sqlite_batch_cycle_and_exports_batch_bundle(self) -> None:
        ...
        config["sources"][0]["options"]["dry_run_events"] = [
            ...
            },
            {
                ...
                "operation": "snapshot",
                "primary_key": {"id": 102},
                "change_version": "main.invoices:2026-04-20T09:01:00Z:102",
                "checkpoint_token": '{"main.invoices":{"watermark":"2026-04-20T09:01:00Z","pk":[102]}}',
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "sqlite-full-cycle-batch-1")
