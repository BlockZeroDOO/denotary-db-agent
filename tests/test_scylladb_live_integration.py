...
from scylladb_live_support import (
    agent_connection_config,
    create_session,
    free_port,
    missing_scylladb_env,
    scylladb_keyspace,
    unique_table_name,
...
class MockIngressHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        response = {
            "request_id": f"scylladb-live-request-{MockIngressHandler.counter}",
            "trace_id": f"scylladb-live-trace-{MockIngressHandler.counter}",
...
class ScyllaDbLiveIntegrationTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        missing = missing_scylladb_env()
        if missing:
            raise unittest.SkipTest(f"missing ScyllaDB live env: {', '.join(missing)}")
    ...
    def _qualified_table(self) -> str:
        return f"{scylladb_keyspace()}.{self.table_name}"
            ...
            "sources": [
                {
                    ...
                    "include": {
                        scylladb_keyspace(): [self.table_name],
                    },
    ...
    def test_live_scylladb_snapshot_and_resume(self) -> None:
        ...
        object_key = f"{scylladb_keyspace()}.{self.table_name}"
    ...
    def test_live_scylladb_backfill_none_skips_until_new_rows_exist(self) -> None:
