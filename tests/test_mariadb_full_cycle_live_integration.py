...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "mariadb-batch-request-1",
                "trace_id": "mariadb-batch-trace-1",
            ...
            response = {
                "request_id": "mariadb-request-1",
                "trace_id": "mariadb-trace-1",
...
class MariaDbFullCycleLiveIntegrationTest(unittest.TestCase):
    @classmethod
    ...
    def test_live_mariadb_full_cycle_exports_proof_bundle(self) -> None:
    ...
    def test_live_mariadb_batch_full_cycle_exports_batch_bundle(self) -> None:
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "mariadb-batch-request-1")
