...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "scylladb-full-cycle-batch-1",
                "trace_id": "scylladb-full-cycle-batch-trace-1",
            ...
            response = {
                "request_id": "scylladb-full-cycle-1",
                "trace_id": "scylladb-full-cycle-trace-1",
...
class ScyllaDbFullCycleTest(unittest.TestCase):
    def setUp(self) -> None:
        MockState.requests = {}
        ...
        config = {
            "agent_name": "scylladb-full-cycle-test",
            "log_level": "INFO",
            "denotary": {
    ...
    def test_run_once_executes_scylladb_full_cycle_and_exports_proof_bundle(self) -> None:
        ...
        deliveries = engine.store.list_deliveries("scylladb-ledger-core")
        self.assertEqual(len(deliveries), 1)
        self.assertEqual(deliveries[0]["status"], "finalized_exported")
        self.assertEqual(deliveries[0]["tx_id"], "c" * 64)

        proofs = engine.store.list_proofs("scylladb-ledger-core")
        ...
        self.assertEqual(proof_payload["request_id"], "scylladb-full-cycle-1")
        self.assertEqual(proof_payload["receipt"]["tx_id"], "c" * 64)
        self.assertEqual(proof_payload["audit_chain"]["record"]["status"], "finalized")

    def test_run_once_executes_scylladb_batch_cycle_and_exports_batch_bundle(self) -> None:
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "scylladb-full-cycle-batch-1")
