...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "snowflake-full-cycle-batch-1",
                "trace_id": "snowflake-full-cycle-batch-trace-1",
            ...
            response = {
                "request_id": "snowflake-full-cycle-1",
                "trace_id": "snowflake-full-cycle-trace-1",
            ...
            state.update(
                {
                    "status": "finalized",
                    "inclusion_verified": True,
                    "finalized_at": "2026-04-20T10:30:00Z",
                }
            )
            response = state
...
class SnowflakeFullCycleTest(unittest.TestCase):
    def setUp(self) -> None:
        MockState.requests = {}
        ...
        config = {
            "agent_name": "snowflake-full-cycle-test",
            "log_level": "INFO",
            "denotary": {
                        ...
                        "dry_run_events": [
                            {
                                ...
                                "operation": "snapshot",
                                "primary_key": {"ID": 101},
                                "change_version": "ANALYTICS.PUBLIC.ORDERS:2026-04-20T09:00:00Z:101",
                                "checkpoint_token": '{"ANALYTICS.PUBLIC.ORDERS":{"watermark":"2026-04-20T09:00:00Z","pk":[101]}}',
                                "commit_timestamp": "2026-04-20T09:00:00Z",
                                "after": {"ID": 101, "STATUS": "issued", "UPDATED_AT": "2026-04-20T09:00:00Z"},
                                "before": None,
                                "metadata": {"capture_mode": "watermark-poll", "object_type": "table"},
                            }
                        ],
                    },
                }
    ...
    def test_run_once_executes_snowflake_full_cycle_and_exports_proof_bundle(self) -> None:
        ...
        deliveries = engine.store.list_deliveries("snowflake-analytics-core")
        self.assertEqual(len(deliveries), 1)
        self.assertEqual(deliveries[0]["status"], "finalized_exported")
        self.assertEqual(deliveries[0]["tx_id"], "c" * 64)

        proofs = engine.store.list_proofs("snowflake-analytics-core")
        ...
        self.assertEqual(proof_payload["request_id"], "snowflake-full-cycle-1")
        self.assertEqual(proof_payload["receipt"]["tx_id"], "c" * 64)
        self.assertEqual(proof_payload["audit_chain"]["record"]["status"], "finalized")

    def test_run_once_executes_snowflake_batch_cycle_and_exports_batch_bundle(self) -> None:
        ...
        config["sources"][0]["options"]["dry_run_events"] = [
            ...
            },
            {
                ...
                "operation": "snapshot",
                "primary_key": {"ID": 102},
                "change_version": "ANALYTICS.PUBLIC.ORDERS:2026-04-20T09:01:00Z:102",
                "checkpoint_token": '{"ANALYTICS.PUBLIC.ORDERS":{"watermark":"2026-04-20T09:01:00Z","pk":[102]}}',
                "commit_timestamp": "2026-04-20T09:01:00Z",
                "after": {"ID": 102, "STATUS": "paid", "UPDATED_AT": "2026-04-20T09:01:00Z"},
                "before": None,
                "metadata": {"capture_mode": "watermark-poll", "object_type": "table"},
            },
        ]
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "snowflake-full-cycle-batch-1")
