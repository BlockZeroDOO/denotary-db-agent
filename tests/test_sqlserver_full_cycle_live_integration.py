...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "sqlserver-batch-request-1",
                "trace_id": "sqlserver-batch-trace-1",
            ...
            response = {
                "request_id": "sqlserver-request-1",
                "trace_id": "sqlserver-trace-1",
...
class SqlServerFullCycleLiveIntegrationTest(unittest.TestCase):
    @classmethod
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    values (9001, N'issued', 111.00, '2026-04-18T10:00:01')
                    """
                )
        finally:
            connection.close()
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    insert into dbo.invoices (id, status, amount, updated_at)
                    values
                        (9101, N'issued', 121.00, '2026-04-18T10:10:01'),
                        (9102, N'paid', 122.00, '2026-04-18T10:10:02')
                    """
                )
        finally:
            connection.close()

    def test_live_sqlserver_full_cycle_exports_proof_bundle(self) -> None:
    ...
    def test_live_sqlserver_batch_full_cycle_exports_batch_bundle(self) -> None:
        self._write_config(
