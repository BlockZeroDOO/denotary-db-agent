...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "snowflake-live-batch-request-1",
                "trace_id": "snowflake-live-batch-trace-1",
            ...
            response = {
                "request_id": "snowflake-live-request-1",
                "trace_id": "snowflake-live-trace-1",
            ...
            state.update(
                {
                    "status": "finalized",
                    "inclusion_verified": True,
                    "finalized_at": "2026-04-20T12:45:00Z",
                }
            )
            response = state
...
class SnowflakeFullCycleLiveIntegrationTest(unittest.TestCase):
    @classmethod
    ...
    def setUp(self) -> None:
        ...
        for thread in self.threads:
            thread.start()
        self.table_name = unique_table_name("DENOTARY_AGENT_FC")
        self._create_table()
    ...
    def _write_config(
        ...
        *,
        ...
        config = {
            "agent_name": "snowflake-full-cycle-live-test",
            "log_level": "INFO",
            "denotary": {
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    values (9001, 'issued', '2026-04-20 13:00:01')
                    """
                )
        finally:
            connection.close()
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute(
                    ...
                    insert into {self.table_name} (ID, STATUS, UPDATED_AT)
                    values
                        (9101, 'issued', '2026-04-20 13:10:01'),
                        (9102, 'paid', '2026-04-20 13:10:02')
                    """
                )
        finally:
            connection.close()

    def test_live_snowflake_full_cycle_exports_proof_bundle(self) -> None:
        ...
        self.assertEqual(proof_payload["request_id"], "snowflake-live-request-1")
        self.assertEqual(proof_payload["receipt"]["tx_id"], "c" * 64)
        self.assertEqual(proof_payload["audit_chain"]["record"]["status"], "finalized")

    def test_live_snowflake_batch_full_cycle_exports_batch_bundle(self) -> None:
        self._write_config(
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "snowflake-live-batch-request-1")
