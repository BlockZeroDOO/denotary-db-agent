...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "oracle-batch-request-1",
                "trace_id": "oracle-batch-trace-1",
...
class OracleFullCycleLiveIntegrationTest(unittest.TestCase):
    @classmethod
    ...
    def _seed_batch_rows(self) -> None:
        ...
        try:
            ...
            cursor.executemany(
                ...
                """,
                [
                    (9101, "issued", 121.00, datetime(2026, 4, 18, 10, 10, 1)),
                    (9102, "paid", 122.00, datetime(2026, 4, 18, 10, 10, 2)),
                ],
            )
            connection.commit()
        finally:
    ...
    def test_live_oracle_full_cycle_exports_proof_bundle(self) -> None:
    ...
    def test_live_oracle_batch_full_cycle_exports_batch_bundle(self) -> None:
        self._write_config(
