...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "redis-full-cycle-batch-1",
                "trace_id": "redis-full-cycle-batch-trace-1",
            ...
            response = {
                "request_id": "redis-full-cycle-1",
                "trace_id": "redis-full-cycle-trace-1",
            ...
            state.update(
                {
                    "status": "finalized",
                    "inclusion_verified": True,
                    "finalized_at": "2026-04-20T11:15:00Z",
                }
            )
            response = state
...
class RedisFullCycleTest(unittest.TestCase):
    def setUp(self) -> None:
        MockState.requests = {}
        ...
        config = {
            "agent_name": "redis-full-cycle-test",
            "log_level": "INFO",
            "denotary": {
                        ...
                        "dry_run_events": [
                            {
                                ...
                                "commit_timestamp": "2026-04-20T11:00:00Z",
                                "after": {"key": "orders:101", "type": "string", "value": "issued"},
                                "before": None,
                                "metadata": {"capture_mode": "scan", "database_number": 0, "key_pattern": "orders:*", "key_type": "string"},
                            }
                        ],
                    },
                }
    ...
    def test_run_once_executes_redis_full_cycle_and_exports_proof_bundle(self) -> None:
        ...
        proofs = engine.store.list_proofs("redis-cache-core")
        ...
        self.assertEqual(proof_payload["request_id"], "redis-full-cycle-1")
        self.assertEqual(proof_payload["receipt"]["tx_id"], "c" * 64)
        self.assertEqual(proof_payload["audit_chain"]["record"]["status"], "finalized")

    def test_run_once_executes_redis_batch_cycle_and_exports_batch_bundle(self) -> None:
        ...
        config["sources"][0]["options"]["dry_run_events"] = [
            ...
            },
            {
                ...
                "table_or_collection": "orders:*",
                "operation": "snapshot",
                "primary_key": {"key": "orders:102"},
                "change_version": "db0:orders:102",
                "checkpoint_token": '{"db0:orders:*":{"last_key":"orders:102"}}',
                "commit_timestamp": "2026-04-20T11:01:00Z",
                "after": {"key": "orders:102", "type": "hash", "value": {"status": "paid", "currency": "EUR"}},
                "before": None,
                "metadata": {"capture_mode": "scan", "database_number": 0, "key_pattern": "orders:*", "key_type": "hash"},
            },
        ]
        self.config_path.write_text(json.dumps(config), encoding="utf-8")
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "redis-full-cycle-batch-1")
