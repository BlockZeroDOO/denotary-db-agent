...
from cassandra_live_support import (
    agent_connection_config,
    cassandra_keyspace,
    create_session,
    free_port,
    missing_cassandra_env,
    unique_table_name,
...
class MockIngressHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        response = {
            "request_id": f"cassandra-live-request-{MockIngressHandler.counter}",
            "trace_id": f"cassandra-live-trace-{MockIngressHandler.counter}",
...
class CassandraLiveIntegrationTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        missing = missing_cassandra_env()
        if missing:
            raise unittest.SkipTest(f"missing Cassandra live env: {', '.join(missing)}")
    ...
    def _qualified_table(self) -> str:
        return f"{cassandra_keyspace()}.{self.table_name}"
    ...
    def _drop_table(self) -> None:
        cluster, session = create_session()
        try:
            session.execute(f"drop table if exists {self._qualified_table()}")
        finally:
            cluster.shutdown()

    def _write_config(self, backfill_mode: str = "full", row_limit: int = 100) -> None:
        config = {
            "agent_name": "live-cassandra-test",
            "log_level": "INFO",
            "denotary": {
            ...
            "sources": [
                {
                    ...
                    "include": {
                        cassandra_keyspace(): [self.table_name],
                    },
    ...
    def _seed_initial_rows(self) -> None:
        ...
        try:
            session.execute(
                ...
                values (1001, 'draft', '2026-04-20T12:00:01Z')
                """
            )
            session.execute(
                f"""
                insert into {self._qualified_table()} (id, status, updated_at)
                values (1002, 'issued', '2026-04-20T12:00:03Z')
                """
            )
        finally:
            cluster.shutdown()
    ...
    def _insert_incremental_row(self) -> None:
        ...
        try:
            session.execute(
                f"""
                insert into {self._qualified_table()} (id, status, updated_at)
                values (1003, 'paid', '2026-04-20T12:00:04Z')
                """
            )
        finally:
            cluster.shutdown()

    def test_live_cassandra_snapshot_and_resume(self) -> None:
        ...
        object_key = f"{cassandra_keyspace()}.{self.table_name}"
    ...
    def test_live_cassandra_backfill_none_skips_until_new_rows_exist(self) -> None:
