...
class MockIngressHandler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:  # noqa: N802
        ...
        if self.path.endswith("/v1/batch/prepare"):
            response = {
                "request_id": "redis-batch-request-1",
                "trace_id": "redis-batch-trace-1",
            ...
            state.update(
                {
                    "status": "finalized",
                    "inclusion_verified": True,
                    "finalized_at": "2026-04-20T13:15:00Z",
                }
            )
            response = state
...
class RedisFullCycleLiveIntegrationTest(unittest.TestCase):
    @classmethod
    ...
    def _write_config(
        ...
        *,
        ...
        config = {
            "agent_name": "redis-full-cycle-live-test",
            "log_level": "INFO",
            "denotary": {
    ...
    def _seed_single_key(self) -> None:
        client = self._connect()
        try:
            client.set("orders:9001", "issued")
        finally:
            close = getattr(client, "close", None)
            if callable(close):
                close()

    def _seed_batch_keys(self) -> None:
        client = self._connect()
        try:
            client.set("orders:9101", "issued")
            client.hset("orders:9102", mapping={"status": "paid", "currency": "EUR"})
        finally:
            close = getattr(client, "close", None)
            if callable(close):
                close()

    def test_live_redis_full_cycle_exports_proof_bundle(self) -> None:
        self._seed_single_key()
        ...
        self.assertEqual(proof_payload["request_id"], "redis-request-1")
        self.assertEqual(proof_payload["receipt"]["tx_id"], "c" * 64)
        self.assertEqual(proof_payload["audit_chain"]["record"]["status"], "finalized")

    def test_live_redis_batch_full_cycle_exports_batch_bundle(self) -> None:
        self._write_config(
        ...
        proof_payload = json.loads(Path(str(export_path)).read_text(encoding="utf-8"))
        self.assertEqual(proof_payload["request_id"], "redis-batch-request-1")
