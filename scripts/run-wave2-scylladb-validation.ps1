...
try {
  ...
  if (-not $ready) {
    throw "ScyllaDB validation container did not become ready in time."
  }
  ...
  C:\Python39\python.exe scripts\run-wave2-scylladb-validation.py --mode $Mode --cycles $Cycles --events-per-cycle $EventsPerCycle
  exit $LASTEXITCODE
}
finally {
