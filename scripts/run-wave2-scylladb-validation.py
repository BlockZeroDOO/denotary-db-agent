...
class IncrementingIngressHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        response = {
            "request_id": f"wave2-scylladb-request-{IncrementingIngressHandler.counter}",
            "trace_id": f"wave2-scylladb-trace-{IncrementingIngressHandler.counter}",
    ...
    def do_POST(self) -> None:  # noqa: N802
        if self.path == "/v1/chain/get_info":
            ...
            payload = {
                ...
                "processed": {"block_num": 982000 + IncrementingIngressHandler.counter},
            }
        else:
            self.send_error(HTTPStatus.NOT_FOUND)
            return
...
def restart_validation() -> dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp:
            ...
            source_id = "scylladb-wave2-restart"
            ...
            if result["baseline_processed"] != 0:
                raise RuntimeError("scylladb restart validation expected a zero-event baseline run")
            if result["first_processed"] != 3 or result["second_processed"] != 3:
                raise RuntimeError("scylladb restart validation did not process 3 + 3 events")
            if result["delivery_count"] != 6 or result["proof_count"] != 6 or result["dlq_count"] != 0:
                raise RuntimeError("scylladb restart validation produced unexpected delivery/proof/DLQ counts")
            return result
    finally:
        stack.stop()
...
def short_soak_validation(*, cycles: int, events_per_cycle: int) -> dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory() as temp:
            ...
            create_table(table_name)
            source_id = "scylladb-wave2-short-soak"
            config_path = build_config(temp_dir, stack, source_id=source_id, table_name=table_name)
            try:
                engine = AgentEngine(load_config(config_path))
                try:
            ...
            if result["baseline_processed"] != 0:
                raise RuntimeError("scylladb short-soak validation expected a zero-event baseline run")
            if total_processed != expected_total or total_failed != 0:
                raise RuntimeError("scylladb short-soak validation did not process the expected number of events")
            if result["delivery_count"] != expected_total or result["proof_count"] != expected_total or result["dlq_count"] != 0:
                raise RuntimeError("scylladb short-soak validation produced unexpected delivery/proof/DLQ counts")
            return result
    finally:
        stack.stop()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Wave 2 ScyllaDB validation")
