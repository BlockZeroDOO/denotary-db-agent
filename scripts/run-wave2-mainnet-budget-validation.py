...
def _build_cassandra_config(
    ...
    *,
        ...
        "sources": [
            {
                ...
                "adapter": "cassandra",
                "enabled": True,
                "source_instance": "wave2-cassandra-mainnet-budget",
    ...
    path = temp_dir / "cassandra-mainnet-budget-config.json"
    path.write_text(json.dumps(config, indent=2), encoding="utf-8")
    return path
    ...
    with _temporary_env(env):
        ...
        try:
            ...
            for cycle in range(cycles):
                ...
                cycle_root.mkdir(parents=True, exist_ok=True)
                cycle_state_db = cycle_root / "offchain-state.sqlite3"
                table_name = unique_cassandra_table_name("wave2_budget")
                source_id = f"cassandra-wave2-denotary-budget-{cycle + 1:02d}"
                cluster, session = create_cassandra_session()
                try:
                    session.execute(
                        f"""
                ...
                stack.start()
                try:
                    config_path = _build_cassandra_config(
                        cycle_root,
                        stack,
                        source_id=source_id,
                        batch_size=batch_size,
                    ...
                    try:
                        ...
                        baseline = engine.run_once()
                        if baseline["processed"] != 0 or baseline["failed"] != 0:
                            raise RuntimeError(f"cassandra budget baseline was not idle: {baseline}")
                        cycle_time = base_time + timedelta(minutes=cycle * 5)
                        for index in range(1, batch_size + 1):
                            record_id = base_id + cycle * 1000 + index
                            _insert_cassandra_row(
                                table_name,
                                row_id=record_id,
                                status=f"budget-{cycle + 1}-{index}",
                                updated_at=(cycle_time + timedelta(seconds=index)).isoformat().replace("+00:00", "Z"),
                    ...
                    try:
                        ...
                        cluster.shutdown()
            if len(cycle_payloads) != cycles:
                raise RuntimeError(f"cassandra budget run produced only {len(cycle_payloads)} successful cycles")
            latest_payload = cycle_payloads[-1]
            total_dlq = sum(int(payload["dlq_count"]) for payload in cycle_payloads)
            if total_dlq != 0:
                raise RuntimeError(f"cassandra budget run produced DLQ entries: {total_dlq}")
            return {
                "adapter": "cassandra",
                "status": "passed",
                "capture_mode": "watermark",
