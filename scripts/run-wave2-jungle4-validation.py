...
JUNGLE4_RPC_URL = "https://jungle4.cryptolions.io"
JUNGLE4_CHAIN_ID = "73e4385a2708e6d7048834fbc1079f2fabb17b3c125b146af438971e90716c4d"
VERIFICATION_ACCOUNT = "decentrfstor"
BILLING_ACCOUNT = "vadim1111111"
SUBMITTER = "newtestactio"
SUBMITTER_PERMISSION = "owner"
SCHEMA_ID = 1776342316
POLICY_ID = 1776343316
WATCHER_TOKEN = "wave2-jungle4-token"
...
class OffchainStack:
    ...
    def _wait_healthy(self, timeout_sec: float = 30.0) -> None:
        ...
        self._wait_urls(healthz, timeout_sec=timeout_sec)
...
def _redis_client():
    if redis is None:
        raise RuntimeError("redis package is required for Redis Jungle4 validation")
...
def build_denotary_config() -> dict[str, Any]:
    raise RuntimeError("build_denotary_config(stack) must be called with a running off-chain stack")
...
def build_sqlite_config(temp_dir: Path, database_path: Path, stack: OffchainStack) -> Path:
    config = {
        "agent_name": "wave2-sqlite-jungle4-validation",
        "log_level": "INFO",
        ...
        "sources": [
            {
                "id": "sqlite-wave2-jungle4",
                "adapter": "sqlite",
                "enabled": True,
                "source_instance": "edge-device-jungle4",
    ...
    path = temp_dir / "sqlite-jungle4-config.json"
...
def build_redis_config(temp_dir: Path, stack: OffchainStack) -> Path:
    config = {
        "agent_name": "wave2-redis-jungle4-validation",
        "log_level": "INFO",
        "denotary": build_denotary_config_for_stack(stack),
        "storage": {
            "state_db": str((temp_dir / "state.sqlite3").resolve()),
        ...
        "sources": [
            {
                "id": "redis-wave2-jungle4",
                "adapter": "redis",
                "enabled": True,
                "source_instance": "cache-jungle4",
                "database_name": "db0",
                "include": {"0": ["orders:*"]},
                "checkpoint_policy": "after_ack",
                "backfill_mode": "full",
    ...
    path = temp_dir / "redis-jungle4-config.json"
    path.write_text(json.dumps(config), encoding="utf-8")
    return path
...
def run_sqlite_validation() -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as temp:
        ...
        try:
            ...
            try:
                marker = build_run_marker("sqlite-jungle4")
                doctor = engine.doctor("sqlite-wave2-jungle4")
                if doctor["overall"]["severity"] == "critical":
                    raise RuntimeError(f"sqlite Jungle4 doctor failed: {doctor['errors']}")
                ...
                proofs = engine.store.list_proofs("sqlite-wave2-jungle4")
                deliveries = engine.store.list_deliveries("sqlite-wave2-jungle4")
                if result["processed"] != 1 or result["failed"] != 0:
                    raise RuntimeError("sqlite Jungle4 run did not process exactly one event")
                delivery = deliveries[0]
                if len(deliveries) != 1:
                    raise RuntimeError("sqlite Jungle4 run did not produce exactly one delivery record")
                proof = proofs[0] if proofs else None
                proof_bundle = load_proof_bundle(proof) if proof is not None else {}
                receipt = dict(proof_bundle.get("receipt") or {})
                watcher_request = engine.watcher.get_request(str(delivery["request_id"]))
                return {
                    "adapter": "sqlite",
                    "status": "passed",
                    ...
                    "block_num": int(watcher_request.get("block_num") or receipt.get("block_num") or 0),
                    "proof_path": proof["export_path"] if proof is not None else None,
                    "broadcast_backend": doctor["signer"]["effective_broadcast_backend"],
                    "finality_mode": "broadcast_only",
                }
            finally:
                engine.close()
        finally:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        try:
            ...
            try:
                ...
                doctor = engine.doctor("redis-wave2-jungle4")
                if doctor["overall"]["severity"] == "critical":
                    raise RuntimeError(f"redis Jungle4 doctor failed: {doctor['errors']}")
                baseline = engine.run_once()
                write_redis_key(str(marker["key"]), str(marker["value"]))
                result = engine.run_once()
                proofs = engine.store.list_proofs("redis-wave2-jungle4")
                deliveries = engine.store.list_deliveries("redis-wave2-jungle4")
                if result["processed"] != 1 or result["failed"] != 0:
                    raise RuntimeError("redis Jungle4 run did not process exactly one event")
                delivery = deliveries[0]
                if len(deliveries) != 1:
                    raise RuntimeError("redis Jungle4 run did not produce exactly one delivery record")
                proof = proofs[0] if proofs else None
                proof_bundle = load_proof_bundle(proof) if proof is not None else {}
                receipt = dict(proof_bundle.get("receipt") or {})
                watcher_request = engine.watcher.get_request(str(delivery["request_id"]))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Wave 2 Jungle4 validation")
