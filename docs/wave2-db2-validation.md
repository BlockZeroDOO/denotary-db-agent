# Wave 2 IBM Db2 Validation

This document describes the first deeper local validation layer for the `Wave 2` `IBM Db2` adapter.

Current scope:

- cold restart recovery through a new agent process on the same local state store
- bounded short-soak under sustained row creation in a Docker-backed local Db2 instance

The validation uses:

- the real `Db2` adapter
- a real local Docker-backed `Db2` source
...
```powershell
scripts/run-wave2-db2-validation.ps1 -Mode restart
scripts/run-wave2-db2-validation.ps1 -Mode short-soak -Cycles 5 -EventsPerCycle 3
```

Shell:

```bash
scripts/run-wave2-db2-validation.sh restart
scripts/run-wave2-db2-validation.sh short-soak 5 3
```

Python:

```bash
python scripts/run-wave2-db2-validation.py --mode restart
python scripts/run-wave2-db2-validation.py --mode short-soak --cycles 5 --events-per-cycle 3
```
...
The short-soak validation applies bounded sustained load to the `Db2` adapter.
...
- the `Db2` baseline handles repeated polling cleanly
