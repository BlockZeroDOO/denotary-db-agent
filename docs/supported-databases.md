...
| ScyllaDB | distributed wide-column | Cassandra-compatible snapshot / watermark baseline, later optional Scylla-specific CDC | adapter contract, config surface, shared Cassandra-compatible snapshot baseline, dedicated adapter alias, local full-cycle proof export, Docker-backed live validation, and deployment/runbook layer implemented |
